...
I will create a summary table displaying the minimum, mean, maximum etc. of each numeric variable within the population adjusted dataset. 

```{r}
# Convert summary to a data frame
summary_df <- summary(data_adj) %>%
  as.data.frame() %>%
...
summary_gt <- summary_df %>%
  gt() %>%
  tab_header(
    title = md("**Supplement Table 1: Summary Statistics of Original Dataset and Population Adjusted Dataset**"),
    subtitle = "Descriptive Summary for Variables"
  ) %>%
  tab_style(
...
manufacturer_yearly_plot_dist_adj <- ggplot(manufacturer_yearly_dist_adj, aes(x = Year, y = Doses, color = Manufacturer)) +
  ...
  scale_color_manual(values = custom_colors_manu) +
  theme_minimal() +
  ggtitle("Figure Five: Population Adjusted COVID-19 Vaccine Distribution by Primary \n Manufacturers Over Time (2021–2023)") +
  xlab("Year") +
  ylab("Total Doses Distributed per 100k (in Millions)") +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5, family = "Times New Roman"),
...
manufacturer_yearly_plot_admin_adj <- ggplot(manufacturer_yearly_admin_adj, aes(x = Year, y = Doses, color = Manufacturer)) +
  ...
  scale_color_manual(values = custom_colors_manu) +
  theme_minimal() +
  ggtitle("Figure Six: Population Adjusted COVID-19 Vaccine Administration by Primary \n Manufacturers Over Time (2021–2023)") +
  xlab("Year") +
  ylab("Total Doses Administered per 100k (in Millions)") +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5, family = "Times New Roman"),
