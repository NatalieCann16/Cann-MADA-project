...
I asked ChatGPT what some interesting analyses may be and I performed a few. 
...
*K-Means Clustering*
I use K-Means Clustering to assess patterns in vaccination trends across different time periods.

```{r}
# Check for NA, NaN, or Inf in the relevant columns
any(is.na(covid_vaccine_combined$Cumulative_Administered))
any(is.nan(covid_vaccine_combined$Cumulative_Administered))
any(is.infinite(covid_vaccine_combined$Cumulative_Administered))

any(is.na(flu_vaccine_all$Cumulative_Flu_Doses_Distributed_Millions))
any(is.nan(flu_vaccine_all$Cumulative_Flu_Doses_Distributed_Millions))
any(is.infinite(flu_vaccine_all$Cumulative_Flu_Doses_Distributed_Millions))
...
# Check the structure of combined_data_clean
str(combined_data_clean)

# Ensure relevant columns are numeric
combined_data_clean <- data.frame(
  Cumulative_Administered = as.numeric(combined_data_clean$Cumulative_Administered),
  Cumulative_Flu_Doses_Distributed_Millions = as.numeric(combined_data_clean$Cumulative_Flu_Doses_Distributed_Millions)
)

# Perform k-means clustering on the cleaned numeric data
clusters <- kmeans(combined_data_clean, centers = 3)
combined_data_clean$Cluster <- as.factor(clusters$cluster)

# Visualize the cleaned data
ggplot(combined_data_clean, aes(x = Cumulative_Administered, y = Cumulative_Flu_Doses_Distributed_Millions, color = Cluster)) +
  geom_point() + theme_minimal()
```
With the three clusters specified in the code above, we see the distribution by which the data points are clustered. 
